...
- 开机自启动：设置弹窗新增开关，登记到系统登录项（Windows 注册表 Run 键 / macOS LaunchAgent），开关状态以系统为准。
- 静默启动：设置弹窗新增开关（默认开启）。开启时无论开机自启还是手动打开，都直接进入后台指示器（菜单栏圆环 / 悬浮球）、不弹主窗口；关闭时启动即显示主窗口（首次启动始终显示界面）。
- 菜单栏面板：点击托盘图标在图标下方弹出紧凑信息面板（仿菜单栏 popover），密集展示当前账号、额度（已用 / 剩余 / 进度条 / 重置倒计时）与服务指标（错误率 / 延迟 / RPM / TPM），并提供「打开主面板」「设置」入口；点击面板外区域自动关闭。

### 优化

- 监控 / 用量标签切换与数据首次加载：标签内容淡入、数据淡入，并在首次 / 切换加载时显示顶部加载条，不再「突然加载完成蹦出来」。
- 主面板隐藏滚动条、用量「设备」选择框固定宽度：切换标签或数据加载导致内容尺寸变化时不再横向跳动。
- 设置弹窗超过窗口高度时内部滚动（头部固定），不再撑出窗口、裁掉底部按钮。

### 修复

- 开机 / 冷启动进入菜单栏圆环模式时，首次拉取额度常因网络未就绪而失败并指数退避，导致菜单栏长时间停在默认 App 图标、与设置里的「菜单栏圆环」不一致 → 现进入圆环模式即刻渲染占位环（菜单栏从第一帧起就是环形），并在首次成功前改用快速重试，真实百分比环在网络就绪后尽快替换占位环。
...
- Release workflow 改为推送 `v*` tag 后直接创建公开 GitHub Release。
- Release workflow 新增 Gitee Release 同步，自动上传桌面端构建产物（需要配置 `GITEE_TOKEN`）。
